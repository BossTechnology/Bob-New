// §5.9 POST /api/notify/email — send alert email via Resend (service).
...
export async function POST(request: NextRequest) {
  try {
    ...
    await notificationService.sendEmail(data as BobAlert, body.recipients)
    return ok({ sent: body.recipients.length })
  } catch (e) {
    return handle(e)
  }
}
